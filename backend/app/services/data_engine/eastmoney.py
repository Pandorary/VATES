...
async def search_stock_by_name(name: str) -> dict | None:
    """通过东方财富 suggest API 按名称搜索股票，返回最佳 A 股匹配
    返回 {"code": "603501", "name": "豪威集团"} 或 None
    """
    try:
        url = (
            "https://searchapi.eastmoney.com/api/suggest/get"
            f"?input={name}&type=14&token=D43BF722C8E33BDC906FB84D85E326E8&count=5"
        )
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if items:
                return {
                    "code": str(items[0].get("Code", "")),
                    "name": str(items[0].get("Name", "")),
                }
            return None
    except Exception as e:
        logger.warning(f"股票名称搜索失败 [{name}]: {e}")
        return None
