...
{"type":"stock|industry|unknown","name":"标准化名称","code":"6位股票代码"}

注意：
- type=stock 时，code 字段必填（6位数字字符串），如果输入是股票名称你必须给出对应的股票代码
- type=industry 或 unknown 时，code 字段为 null

示例：
- "600519" → {"type":"stock","name":"贵州茅台","code":"600519"}
- "贵州茅台" → {"type":"stock","name":"贵州茅台","code":"600519"}
- "比亚迪" → {"type":"stock","name":"比亚迪","code":"002594"}
- "白酒" → {"type":"industry","name":"白酒","code":null}
- "天气" → {"type":"unknown","name":"天气","code":null}"""
...
def _parse_classify_json(raw: str, fallback_name: str) -> dict:
    ...
    try:
        ...
        code = data.get("code") if type_ == "stock" else None
        return {"type": type_, "name": str(name), "code": code}
    ...
    return {"type": "unknown", "name": fallback_name, "code": None}
